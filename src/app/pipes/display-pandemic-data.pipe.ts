...
export class DisplayPandemicDataPipe implements PipeTransform {
  ...
  transform(value: PandemicData, iWantTheTruth?: boolean): string {
    ...
    if (iWantTheTruth) {
      ...
      newCases = `, újonnan azonosított fertőzöttek száma ${Math.round(Math.min(Math.random() * 500, value.newTests! * 0.05))}`;
      inHospital = Math.round(value.inHospital! / 3);
    }

    return `napon a mintavételek száma ${newTests}${newCases}; aminek következtében ${inHospital}-an vannak kórházban${iWantTheTruth ? '.' : ' (mindannyian oltatlan idős krónikus betegek).'}`;
  }
